  ...
  async sync(mitraId: string, items: HistoryItemDto[]): Promise<SyncHistoryResult> {
    ...
    const rows = items.map((item, index) => {
      ...
      return `($${offset + 1}::uuid, $${offset + 2}::uuid, $${offset + 3}::bigint,
        ...
        $${offset + 13}::jsonb, $${offset + 14}::bigint, $${offset + 15}::varchar,
        $${offset + 16}::varchar, $${offset + 17}::numeric, $${offset + 18}::numeric)`;
    });
      ...
      const rawInserted: unknown = await manager.query(
        `INSERT INTO history
          (uuid, mitra_id, id_produk, terjual, harga, created_at, keterangan,
           ...
           raw_material_addons, discount_id, discount_name, discount_type, discount_value, discount_amount)
         VALUES ${rows.join(',')}
