...
export function InventoryScreen({ session, refreshKey = 0, onRefreshed }: { session: Session; refreshKey?: number; onRefreshed?: () => void }) {
  ...
  const [rawAdjustmentType, setRawAdjustmentType] = useState<'add' | 'subtract'>('subtract');
  ...
  const adjustRaw = async () => {
    ...
    if (!Number.isFinite(normalized) || normalized <= 0) return Alert.alert('Jumlah belum valid', 'Masukkan jumlah perubahan stok.');
    if (rawAdjustmentType === 'subtract' && normalized > reducing.stock) return Alert.alert('Stok tidak cukup', `Maksimal yang dapat dikurangi ${reducing.stock} ${reducing.unit || 'unit'}.`);
    setSaving(true);
    try { await adjustLocalStock(session.mitraId, reducing.id, rawAdjustmentType === 'add' ? normalized : -normalized); await load(); setReducing(null); }
  ...
  return <View style={styles.screen}>
    ...
    <Modal visible={Boolean(reducing)} transparent animationType="fade" onRequestClose={() => setReducing(null)}><View style={styles.backdrop}><View style={styles.modal}><Text style={styles.modalTitle}>{rawAdjustmentType === 'add' ? 'Tambah' : 'Kurangi'} {reducing?.name}</Text><Text style={styles.label}>Jumlah perubahan</Text><TextInput value={rawAmount} onChangeText={(value) => setRawAmount(value.replace(/[^0-9.,]/g, ''))} keyboardType="decimal-pad" placeholder="Contoh: 200" placeholderTextColor="#98A2B3" style={styles.input} /><View style={styles.unitOptions}>{unitsFor(reducing?.unit).map((unit) => <Pressable key={unit} onPress={() => setRawUnit(unit)} style={[styles.unitOption, rawUnit === unit && styles.unitOptionActive]}><Text style={[styles.unitOptionText, rawUnit === unit && styles.unitOptionTextActive]}>{unit}</Text></Pressable>)}</View><Text style={styles.hint}>Stok tersedia: {reducing ? Number(reducing.stock.toFixed(3)) : 0} {reducing?.unit || 'unit'}.</Text><View style={styles.actions}><Pressable onPress={() => setReducing(null)} style={styles.cancel}><Text style={styles.cancelText}>Batal</Text></Pressable><Pressable disabled={saving} onPress={() => void adjustRaw()} style={[styles.save, saving && { opacity: .5 }]}><Text style={styles.saveText}>{saving ? 'Menyimpan...' : rawAdjustmentType === 'add' ? 'Tambah stok' : 'Kurangi stok'}</Text></Pressable></View></View></View></Modal>
  </View>;
}
