...
import { createProcurementOrder, finalizeProcurementOrder, getMyOrders, RemoteOrder, updatePendingProcurementOrder } from '../procurement/procurement-api';
import { downloadOrderInvoice } from '../procurement/invoice';
...
const procurementDrafts = new Map<string, { quantities: Record<number, string>; selectedIds: number[]; paymentMethod: 'tunai' | 'qris' | null }>();
...
export function OrderScreen({ isTablet, session }: { isTablet: boolean; session: Session }) {
  ...
  const initialDraft = procurementDrafts.get(session.mitraId);
  const [quantities, setQuantities] = useState<Record<number, string>>(() => initialDraft?.quantities ?? {});
  ...
  const [paymentMethod, setPaymentMethod] = useState<'tunai' | 'qris' | null>(() => initialDraft?.paymentMethod ?? null);
  const [editingOrderId, setEditingOrderId] = useState<string | null>(null);

  useEffect(() => NetInfo.addEventListener((state) => setOnline(Boolean(state.isConnected) && state.isInternetReachable !== false)), []);
  useEffect(() => { procurementDrafts.set(session.mitraId, { quantities, selectedIds, paymentMethod }); }, [quantities, selectedIds, paymentMethod, session.mitraId]);
  ...
  const submit = async () => {
    ...
    if (!paymentMethod) return Alert.alert('Pilih metode pembayaran', 'Pilih pembayaran Tunai atau QRIS sebelum membuat pesanan.');
    setSubmitting(true);
    try {
      const orderLines = lines.map((line) => ({ warehouseId: line.product.id, quantity: line.quantity, unit: line.unit }));
      const created = editingOrderId
        ? await updatePendingProcurementOrder(session, editingOrderId, orderLines, paymentMethod)
        : await createProcurementOrder(session, orderLines, paymentMethod);
      ...
      await loadOrders();
      setQuantities({}); setSelectedIds([]); setPaymentMethod(null); setEditingOrderId(null); setTab('status');
  ...
  const editOrder = async (order: RemoteOrder) => {
    if (order.status !== 'pending') return;
    if (!online) return Alert.alert('Koneksi internet diperlukan', 'Edit pesanan membutuhkan akses internet.');
    await loadCatalog();
    setSelectedIds(order.items.map((item) => Number(item.warehouseId)));
    setQuantities(Object.fromEntries(order.items.map((item) => [Number(item.warehouseId), String(item.jumlahPesan)])));
    setPaymentMethod(order.paymentMethod || 'tunai');
    setEditingOrderId(order.id); setTab('catalog'); setCartVisible(true);
  };
  const finalizeOrder = async (order: RemoteOrder) => {
    try { const updated = await finalizeProcurementOrder(session, order.id); setOrders((current) => current.map((item) => item.id === updated.id ? updated : item)); Alert.alert('Pesanan sudah fix', 'Pesanan dikunci dan siap diproses oleh Pusat.'); }
    catch (error) { Alert.alert('Pesanan gagal dikunci', error instanceof Error ? error.message : 'Terjadi kesalahan'); }
  };
  ...
  return <View style={styles.screen}>
    ...
    {tab === 'catalog' ? <View style={styles.catalogWrap}>
      {editingOrderId && <View style={styles.editingBanner}><Text style={styles.editingText}>Mengedit Invoice PO #{editingOrderId}</Text><Pressable onPress={() => { setEditingOrderId(null); setSelectedIds([]); setQuantities({}); setPaymentMethod(null); }}><Text style={styles.editingCancel}>Batal</Text></Pressable></View>}
      ...
      {catalogLoading ? <ActivityIndicator color={colors.primary} /> : catalogError ? <ErrorCard message={catalogError} online={online} retry={loadCatalog} /> : <FlatList data={visibleCatalog} key={isTablet ? 'tablet-order' : 'phone-order'} numColumns={isTablet ? 4 : 2} keyExtractor={(item) => String(item.id)} columnWrapperStyle={styles.gridRow} contentContainerStyle={styles.grid} ListEmptyComponent={<Text style={styles.empty}>Belum ada bahan baku pada kategori ini.</Text>} renderItem={({ item }) => <View style={styles.productWrap}>
        <ProductCard product={item} onAdd={selectProduct} compact showPrice hideStock ignoreStock selectedQuantity={Number(quantities[item.id]) || 0} />
...
const styles = StyleSheet.create({
  ...
  orderList: { margin: 16, backgroundColor: colors.surface, borderRadius: 16, paddingHorizontal: 14 }, orderRow: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: colors.line }, orderInfo: { flex: 1 }, invoiceLabel: { color: colors.primary, fontSize: 9, fontWeight: '900', letterSpacing: 1.5 }, orderId: { color: colors.ink, fontWeight: '900' }, orderMeta: { color: colors.muted, fontSize: 11, marginTop: 4 }, orderTotal: { color: colors.ink, fontSize: 16, fontWeight: '900', marginTop: 7 }, editOrder: { alignSelf: 'flex-start', marginTop: 9, paddingHorizontal: 11, paddingVertical: 7, backgroundColor: colors.primarySoft, borderRadius: 8 }, editOrderText: { color: colors.primary, fontSize: 10, fontWeight: '900' }, status: { flexDirection: 'row', gap: 6, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 999, alignItems: 'center' }, statusDot: { width: 6, height: 6, borderRadius: 3 }, statusText: { fontWeight: '800', fontSize: 11, textTransform: 'capitalize' },
  selectedBadge: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, padding: 7, backgroundColor: colors.primarySoft, borderRadius: 9, marginTop: 6 }, selectedText: { color: colors.primary, fontSize: 9, fontWeight: '900' }, orderActions: { flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 8 }, invoiceButton: { paddingHorizontal: 10, paddingVertical: 7, borderRadius: 8, backgroundColor: colors.canvas }, invoiceButtonText: { color: colors.ink, fontSize: 9, fontWeight: '900' }, fixOrder: { paddingHorizontal: 10, paddingVertical: 7, borderRadius: 8, backgroundColor: colors.primary }, fixOrderText: { color: '#FFF', fontSize: 9, fontWeight: '900' },
  editingBanner: { marginHorizontal: 16, marginTop: 10, padding: 11, borderRadius: 10, backgroundColor: colors.primarySoft, flexDirection: 'row', justifyContent: 'space-between' }, editingText: { color: colors.primary, fontWeight: '900', fontSize: 11 }, editingCancel: { color: colors.red, fontWeight: '900', fontSize: 11 }, modalBackdrop: { flex: 1, backgroundColor: 'rgba(16,24,40,.6)', justifyContent: 'flex-end' }, cartModal: { width: '100%', maxWidth: 620, maxHeight: '85%', alignSelf: 'center', backgroundColor: colors.surface, borderTopLeftRadius: 22, borderTopRightRadius: 22 }, modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 18, borderBottomWidth: 1, borderBottomColor: colors.line }, modalTitle: { color: colors.ink, fontSize: 19, fontWeight: '900' }, cartContent: { padding: 16 }, cartRow: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.line }, cartInfo: { flex: 1 }, cartName: { color: colors.ink, fontWeight: '900' }, cartMeta: { color: colors.muted, fontSize: 10, marginTop: 3 }, cartInput: { width: 78, height: 42, borderWidth: 1, borderColor: colors.line, borderRadius: 9, paddingHorizontal: 9, color: colors.ink }, cartUnit: { color: colors.ink, fontWeight: '800', fontSize: 10 }, cartFooter: { padding: 18, borderTopWidth: 1, borderTopColor: colors.line }, paymentLabel: { color: colors.muted, fontWeight: '800', fontSize: 10, marginBottom: 7 }, paymentOptions: { flexDirection: 'row', gap: 8, marginBottom: 15 }, paymentOption: { flex: 1, alignItems: 'center', padding: 11, borderWidth: 1, borderColor: colors.line, borderRadius: 10 }, paymentOptionActive: { borderColor: colors.primary, backgroundColor: colors.primarySoft }, paymentOptionText: { color: colors.muted, fontWeight: '900', fontSize: 11 }, paymentOptionTextActive: { color: colors.primary }, totalRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, totalLabel: { color: colors.muted }, totalValue: { color: colors.ink, fontSize: 20, fontWeight: '900' }, checkout: { backgroundColor: colors.primary, borderRadius: 12, padding: 15, alignItems: 'center', marginTop: 13 }, paymentModal: { width: '92%', maxWidth: 470, maxHeight: '90%', alignSelf: 'center', marginBottom: 'auto', marginTop: 'auto', backgroundColor: colors.surface, borderRadius: 20, padding: 20, alignItems: 'center' }, paymentHint: { color: colors.muted, textAlign: 'center', lineHeight: 17, fontSize: 11, marginTop: 7 }, invoiceLine: { width: '100%', flexDirection: 'row', justifyContent: 'space-between', gap: 8, paddingVertical: 7, borderBottomWidth: 1, borderBottomColor: colors.line }, centralQris: { width: 210, height: 210, marginVertical: 12, backgroundColor: '#FFF', borderRadius: 10 }, qrisEmpty: { width: 210, height: 160, marginVertical: 12, backgroundColor: colors.canvas, borderRadius: 12, alignItems: 'center', justifyContent: 'center' }, paymentTotal: { color: colors.ink, fontSize: 24, fontWeight: '900', marginVertical: 13 }, whatsapp: { width: '100%', backgroundColor: '#25D366', padding: 14, borderRadius: 12, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 8 }, closePayment: { padding: 12 }, closePaymentText: { color: colors.muted, fontWeight: '800' },
});
