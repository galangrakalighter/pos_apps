...
import { editPendingTransaction, getLocalSalesHistory, LocalSale, PaymentMethod } from '../database/sales.repository';
...
export function MitraSalesHistoryScreen({ session }: { session: Session }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { await syncDiscounts(session).catch(() => undefined); setDiscounts(await getLocalDiscounts()); setHistories(await getLocalSalesHistory(session.mitraId)); }
  ...
  return <View style={styles.screen}>
    <FlatList data={transactions} keyExtractor={(item) => item.id} contentContainerStyle={styles.list} refreshControl={<RefreshControl refreshing={loading} onRefresh={() => void load()} />} ListHeaderComponent={<><View style={styles.summary}><Text style={styles.summaryLabel}>Penjualan sesuai filter</Text><Text style={styles.summaryValue}>{rupiah(summary.revenue / 100)}</Text><Text style={styles.summarySub}>{summary.transactions} transaksi · {summary.items} item terjual</Text></View><View style={styles.filters}><Text style={styles.filterTitle}>Filter transaksi</Text><FilterRow label="Periode" value={period} onChange={(value) => setPeriod(value as PeriodFilter)} options={[['today', 'Hari ini'], ['7days', '7 hari'], ['30days', '30 hari'], ['all', 'Semua']]} /><FilterRow label="Sinkronisasi" value={syncFilter} onChange={(value) => setSyncFilter(value as SyncFilter)} options={[['all', 'Semua'], ['pending', 'Pending'], ['synced', 'Synced']]} /><FilterRow label="Pembayaran" value={paymentFilter} onChange={(value) => setPaymentFilter(value as PaymentFilter)} options={[['all', 'Semua'], ['tunai', 'Tunai'], ['qris', 'QRIS']]} /></View></>} ListEmptyComponent={!loading ? <Text style={styles.empty}>{allTransactions.length ? 'Tidak ada transaksi yang sesuai dengan filter.' : 'Belum ada transaksi penjualan.'}</Text> : null} renderItem={({ item }) => <View style={styles.row}>
      ...
      <View style={styles.right}><Text style={styles.amount}>{rupiah(item.totalCents / 100)}</Text><View style={[styles.badge, item.status === 'synced' ? styles.synced : styles.pending]}><Text style={[styles.badgeText, item.status === 'synced' ? styles.syncedText : styles.pendingText]}>{item.status === 'synced' ? 'Synced' : 'Pending Sync'}</Text></View><View style={{flexDirection:'row',gap:6}}>{item.status === 'pending' && <Pressable onPress={() => setEditing(item)} style={styles.detailButton}><Text style={styles.detailText}>Edit</Text></Pressable>}<Pressable onPress={() => setSelected(item)} style={styles.detailButton}><Text style={styles.detailText}>Detail</Text></Pressable></View></View>
    </View>} />
    <TransactionDetail transaction={selected} session={session} onClose={() => setSelected(null)} />
    <EditTransaction transaction={editing} discounts={discounts} ownerId={session.mitraId} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); void load(); }} />
  </View>;
}

function EditTransaction({ transaction, discounts, ownerId, onClose, onSaved }: { transaction: SaleTransaction | null; discounts: Discount[]; ownerId: string; onClose: () => void; onSaved: () => void }) {
  const [quantities, setQuantities] = useState<Record<number, number>>({}); const [discountId, setDiscountId] = useState<number|null>(null); const [saving,setSaving]=useState(false);
  useEffect(() => { if (transaction) { setQuantities(Object.fromEntries(transaction.items.map((x) => [x.product_id, x.sold_quantity]))); setDiscountId(transaction.items[0].discount_id); } }, [transaction]);
  if (!transaction) return null;
  const change=(id:number,delta:number)=>setQuantities((current)=>({...current,[id]:Math.max(0,(current[id]??0)+delta)}));
  const save=async()=>{setSaving(true);try{await editPendingTransaction(ownerId,transaction.id,quantities,discounts.find((x)=>x.id===discountId)??null);onSaved();}catch(e){Alert.alert('Transaksi gagal diedit',e instanceof Error?e.message:'Terjadi kesalahan');}finally{setSaving(false);}};
  return <Modal visible transparent animationType="fade" onRequestClose={onClose}><View style={detailStyles.backdrop}><View style={detailStyles.modal}><View style={detailStyles.header}><Text style={detailStyles.title}>Edit transaksi</Text><Pressable onPress={onClose}><Text style={detailStyles.close}>×</Text></Pressable></View><ScrollView contentContainerStyle={detailStyles.content}><Text style={detailStyles.section}>Jumlah produk</Text>{transaction.items.map((item)=><View key={item.uuid} style={detailStyles.item}><View style={detailStyles.itemInfo}><Text style={detailStyles.itemName}>{item.product_name}</Text><Text style={detailStyles.itemMeta}>{rupiah(item.price_cents/100)} · jumlah 0 akan menghapus item</Text></View><View style={{flexDirection:'row',alignItems:'center',gap:10}}><Pressable onPress={()=>change(item.product_id,-1)} style={styles.detailButton}><Text style={styles.detailText}>−</Text></Pressable><Text style={{fontWeight:'900'}}>{quantities[item.product_id]??0}</Text><Pressable onPress={()=>change(item.product_id,1)} style={styles.detailButton}><Text style={styles.detailText}>+</Text></Pressable></View></View>)}<Text style={detailStyles.section}>Diskon</Text><ScrollView horizontal contentContainerStyle={{gap:7}}><Pressable onPress={()=>setDiscountId(null)} style={[styles.filterChip,discountId===null&&styles.filterChipActive]}><Text style={[styles.filterChipText,discountId===null&&styles.filterChipTextActive]}>Tanpa diskon</Text></Pressable>{discounts.map((item)=><Pressable key={item.id} onPress={()=>setDiscountId(item.id)} style={[styles.filterChip,discountId===item.id&&styles.filterChipActive]}><Text style={[styles.filterChipText,discountId===item.id&&styles.filterChipTextActive]}>{item.name}</Text></Pressable>)}</ScrollView></ScrollView><View style={{flexDirection:'row',gap:8,padding:18}}><Pressable onPress={onClose} style={[detailStyles.done,{flex:1,margin:0,backgroundColor:colors.canvas}]}><Text style={{color:colors.ink,fontWeight:'900'}}>Batal</Text></Pressable><Pressable disabled={saving} onPress={()=>void save()} style={[detailStyles.done,{flex:2,margin:0}]}><Text style={detailStyles.doneText}>{saving?'Menyimpan...':'Simpan perubahan'}</Text></Pressable></View></View></View></Modal>;
}
...
function TransactionDetail({ transaction, session, onClose }: { transaction: SaleTransaction | null; session: Session; onClose: () => void }) {
  if (!transaction) return null;
  const print = () => void printReceipt({
    id: transaction.id, merchantName: session.partnerName || 'POS Mitra', cashierName: session.name,
    createdAt: transaction.createdAt, paymentMethod: paymentLabel(transaction.paymentMethod),
    items: transaction.items.map((item) => ({ name: item.product_name, quantity: item.sold_quantity, priceCents: item.price_cents, addons: parseRawMaterialAddons(item.raw_material_addons).map((addon) => addon.name) })),
    totalCents: transaction.totalCents, amountPaidCents: transaction.amountPaidCents,
    changeCents: transaction.changeCents, discountName: transaction.discountName,
    discountAmountCents: transaction.discountAmountCents, note: transaction.note,
  }).catch((error) => Alert.alert('Struk gagal dicetak', error instanceof Error ? error.message : 'RawBT tidak dapat dibuka'));
  return <Modal visible transparent animationType="fade" onRequestClose={onClose}><View style={detailStyles.backdrop}><View style={detailStyles.modal}>
    ...
    <ScrollView contentContainerStyle={detailStyles.content}>
      ...
      {transaction.items.map((item) => { const addons = parseRawMaterialAddons(item.raw_material_addons); return <View key={item.uuid} style={detailStyles.item}><View style={detailStyles.itemInfo}><Text style={detailStyles.itemName}>{item.product_name}</Text><Text style={detailStyles.itemMeta}>{item.sold_quantity} × {rupiah(item.price_cents / 100)}</Text>{addons.length > 0 && <Text style={detailStyles.itemMeta}>Add-on: {addons.map((addon) => addon.name).join(', ')}</Text>}</View><Text style={detailStyles.itemTotal}>{rupiah(item.price_cents * item.sold_quantity / 100)}</Text></View>; })}
      {transaction.discountName && <View style={detailStyles.addonBox}><Text style={detailStyles.addonLabel}>Diskon digunakan</Text><DetailRow label={transaction.discountName} value={`- ${rupiah(transaction.discountAmountCents / 100)}`} strong /></View>}
    ...
    <View style={detailStyles.footerActions}><Pressable onPress={onClose} style={detailStyles.closeButton}><Text style={detailStyles.closeButtonText}>Tutup</Text></Pressable><Pressable onPress={print} style={detailStyles.printButton}><Text style={detailStyles.doneText}>Cetak struk</Text></Pressable></View>
  </View></View></Modal>;
}
